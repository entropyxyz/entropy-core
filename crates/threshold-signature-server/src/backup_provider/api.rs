...
use entropy_client::{substrate::query_chain, user::check_quote_measurement, attestation::create_quote};
use entropy_shared::{
...
pub async fn request_recover_encryption_key(
    ...
    let key_request = serde_json::to_string(&key_request)?;

    let get_encryption_key = || async {
        tracing::info!("Requesting encryption key from backup provider");
        let client = reqwest::Client::new();
        let response = client
            .post(format!(
