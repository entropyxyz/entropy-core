...
async fn test_get_oracle_data() {
    ...
    let oracle_data = get_oracle_data(&api, &rpc, vec!["block_number_entropy".encode()]).await.unwrap();
    ...
    // fails gracefully
    let oracle_data_fail = get_oracle_data(&api, &rpc, vec!["random_heading".encode()]).await.unwrap();
