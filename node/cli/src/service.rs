  ...
  fn test_sync() {
    ...
    sc_service_test::sync(
      ...
      |config| {
        ...
        let NewFullBase { task_manager, client, network, transaction_pool, .. } = new_full_base(
          config,
        ...
        let node =
          sc_service_test::TestNetComponents::new(task_manager, client, network, transaction_pool);
        ...
        let (babe_pre_digest, epoch_descriptor) = loop {
          ...
          let epoch = babe_link
            ...
            .epoch_data(&epoch_descriptor, |slot| {
              sc_consensus_babe::Epoch::genesis(babe_link.config().genesis_config(), slot)
            })
            .unwrap();
