...
fn get_d_log_statement() -> DLogStatement {
	...
	DLogStatement { N: N_tilde, g: h1, ni: h2 }
}
...
fn share_summands_and_add_elementwise(
	key_summands: Vec<Scalar<Secp256k1>>,
	t: u16,
	n: u16,
	...
	for summand in key_summands {
		let (vss_scheme, secret_shares) = VerifiableSS::share(t, n, &summand);
...
pub(crate) fn split_masterkey_into_summands(
	master_key: &Scalar<Secp256k1>,
	n: usize,
) -> Result<Vec<Scalar<Secp256k1>>, KeygenError> {
	if n < 1 {
...
fn _secret_sharing_proof_of_concept(t: u16, n: u16) {
	...
	for (i, share) in secret_shares.polynomial().evaluate_many_bigint(1..=n).enumerate() {
		...
		println!(
			"party_keys: {} {:?} {:?} {:?}",
			i, &party_keys_vec[i].party_index, &party_keys_vec[i].u_i, &party_keys_vec[i].y_i
		);
	}
}
