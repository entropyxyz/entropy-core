...
// for this we need to change code in pallet::relayer such that relayer adds SigRequest to
// runtime_metadata type SigRequest =  entropy::relayer::SigRequest;
...
impl User {
	pub async fn sign_message(&self, msg: u16) -> Result<()> {
		...
		println!("user starts signing... sign_cli: {:?}", sign_cli);
	...
	pub async fn request_sig_gen(
		&self,
		sig_req: entropy::runtime_types::common::common::SigRequest,
	) -> Result<common::SigResponse> {
		// } , Box<dyn std::error::Error>> {
		//pub async fn request_sig_gen(&self) -> Result<common::SigResponse> { // } , Box<dyn
		// std::error::Error>> {

		println!("request_sig_gen is called");
		let signer = PairSigner::new(AccountKeyring::Alice.pair());
		// ToDo: JA hardcoded URL
		...
		// i.e. define SigResponse in pallet::relayer so that it shows up in entropy's
		// runtime_metadata and then use that type here maybe this way all/many structs in
		// crypto/common can be migrated into the substrate codebase maybe this can help: https://github.com/paritytech/subxt/tree/55f04c20a78c5cb1b26584802f562a0cc8f9eb12/test-runtime
	...
	pub async fn send_registration_request(&self) -> Result<common::RegistrationResponse> {
		...
		let result = api
			.tx()
			.relayer()
			.register(entropy::runtime_types::common::common::RegistrationMessage {
				keyshards: 123,
				test: 369,
			})
			.sign_and_submit_then_watch(&signer)
		...
		let responding_node = result
			...
			.0;
		let reg_response = result
			.find_first_event::<entropy::relayer::events::AccountRegistered>()?
			.context("send_registration_request no result received")?
			.1;

		// let reg_response =
		// result.find_first_event::<entropy::relayer::events::AccountRegistered>()?
		// .context("request_sig_gen no result received")?.1;

		// println!{"reg_response {:?}", reg_response};
		let regres = common::RegistrationResponse { signing_nodes: reg_response.signing_nodes };
