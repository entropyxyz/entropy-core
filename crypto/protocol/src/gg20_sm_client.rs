...
pub async fn join_computation<M>(
	address: surf::Url,
	room_id: &str,
) -> Result<(u16, impl Stream<Item = Result<Msg<M>>>, impl Sink<Msg<M>, Error = anyhow::Error>)>
where
	M: Serialize + DeserializeOwned,
{
	...
	let incoming = client.subscribe().await.context("subscribe")?.and_then(|msg| async move {
...
impl SmClient {
	...
	pub async fn subscribe(&self) -> Result<impl Stream<Item = Result<String>>> {
		let response = self.http_client.get("subscribe").await.map_err(|e| e.into_inner())?;
