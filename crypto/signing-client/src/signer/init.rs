...
//! A [SignInitSanitized] struct is created out of the raw incoming [proto::SignInit] message and
...
// 	/// On success, it extracts the PartyInfo from the KVStrore and returns a sanitized struct ready
// 	/// to be used by the protocol. On failure, returns an [anyhow!] error and no changes are been
// 	/// made in the KvStore.
...
// 					anyhow!("participant [{}] not found in key [{}]", s, sign_init.key_uid)
// 				})
// 			})
...
// 			participant_uids: vec!["party_2".to_owned(), "party_1".to_owned()], /* party 2 has
// 			                                                                     * index 2, party
// 			                                                                     * 1 has index 1 */
// 			participant_indices: vec![2, 1], // indices should be [2, 1]
// 			message_to_sign: vec![42; 32].as_slice().try_into().unwrap(), /* msg of 32 bytes
...
// 		assert_eq!(&res.participant_indices, &sanitized_sign_init.participant_indices);
// 		assert_eq!(&res.message_to_sign, &sanitized_sign_init.message_to_sign);
// 	}

// 	#[test]
// 	fn test_fail_sign_sanitize_args() {
// 		let all_party_uids = vec!["party_0".to_owned(), "party_1".to_owned(), "party_2".to_owned()];
// 		let raw_sign_init = proto::SignInit {
// 			new_sig_uid: "test_uid".to_owned(),
// 			key_uid: "test_uid".to_owned(),
