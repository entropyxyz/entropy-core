...
//! When all sign threads finish, we aggregate their results and retrieve the signature of the
//! message. The signature must be the same across all results.
...
// 		// cannot use aggregator_receivers.map(|aggr| aggr.await??) because map() does not support
// 		// async funcs
