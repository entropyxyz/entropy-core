...
use testing_utils::{
    constants::{X25519_PUBLIC_KEYS, TSS_ACCOUNTS},
    substrate_context::{
...
async fn test_sign_tx_no_chain() {
    ...
    let message_request = Message {
        ...
        validators_info: vec![
            ValidatorInfo { ip_address: b"127.0.0.1:3001".to_vec(), x25519_public_key: [0; 32], tss_account: TSS_ACCOUNTS[0].encode() },
            ValidatorInfo { ip_address: b"127.0.0.1:3002".to_vec(), x25519_public_key: [0; 32], tss_account: TSS_ACCOUNTS[1].encode() },
        ],
    };
...
async fn test_unsigned_tx_endpoint() {
    ...
    let ports_and_keys = vec![(3001, X25519_PUBLIC_KEYS[0], TSS_ACCOUNTS[0].clone()), (3002, X25519_PUBLIC_KEYS[1], TSS_ACCOUNTS[1].clone())];

    let validator_info: Vec<(String, [u8; 32], AccountId32)> = ports_and_keys
        .iter()
        .map(|validator_tuple| (format!("127.0.0.1:{}", validator_tuple.0), validator_tuple.1, validator_tuple.2.clone()))
        .collect::<Vec<_>>();
