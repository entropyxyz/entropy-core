  ...
  K: Clone,
{
  ...
  while let Protocol::NotDone(mut round) = party {
    ...
    party = round.execute_next_round().map_err(|_| anyhow!("Error in tofn::execute_next_round"))?;
  }
...
async fn handle_incoming<F, K, P, const MAX_MSG_IN_LEN: usize>(
  ...
  while round.expecting_more_msgs_this_round() {
    ...
    let traffic = channel_in
      .next()
      .await
      .ok_or(format!("{}: stream closed by client before protocol has completed", round_count));
