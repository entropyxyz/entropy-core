...
impl EncryptedDb {
    ...
    V: Into<IVec>,
  {
    ...
    // encrypt value
    self.cipher.encrypt_in_place(&nonce, b"", &mut value).map_err(|e| Encryption(e.to_string()))?;
  ...
  fn decrypt_record_value(&self, record: EncryptedRecord) -> EncryptedDbResult<IVec> {
    ...
    // decrypt value
    self.cipher.decrypt_in_place(&nonce, b"", &mut value).map_err(|e| Decryption(e.to_string()))?;
