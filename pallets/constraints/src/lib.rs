...
pub mod pallet {
  ...
  impl<T: Config> Pallet<T> {
    ...
    pub fn add_whitelist_address(
      ...
      ensure!(whitelist_addresses.len() as u32 <= T::MaxWhitelist::get(), Error::<T>::MaxWhitelist);
      let whitelist_length = AddressWhitelist::<T>::try_mutate(
        who.clone(),
        |addresses| -> Result<usize, DispatchError> {
          if (addresses.len() as u32 + whitelist_addresses.len() as u32) > T::MaxWhitelist::get() {
