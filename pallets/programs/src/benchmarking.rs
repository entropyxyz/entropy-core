...
benchmarks! {
  ...
  set_program {
    ...
    let _ = CurrencyOf::<T>::make_free_balance_be(&program_deploy_key, value);

  }: _(RawOrigin::Signed(program_deploy_key.clone()), program.clone(), configuration_interface.clone())
  verify {
    assert_last_event::<T>(
  ...
  remove_program {
    ...
    let value = CurrencyOf::<T>::minimum_balance().saturating_mul(1_000_000_000u32.into());
    let _ = CurrencyOf::<T>::make_free_balance_be(&program_deploy_key, value);
    <Programs<T>>::insert(program_hash.clone(), ProgramInfo {bytecode: program, configuration_interface, program_deploy_key: program_deploy_key.clone(), ref_counter: 0u128});
    ...
    <OwnedPrograms<T>>::insert(program_deploy_key.clone(), bounded_program_hashes);
  }: _(RawOrigin::Signed(program_deploy_key.clone()), program_hash.clone())
  verify {
    assert_last_event::<T>(
