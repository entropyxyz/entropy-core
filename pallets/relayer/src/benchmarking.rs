...
benchmarks! {
  ...
  let program_deploy_key: T::AccountId = whitelisted_caller();
    Programs::<T>::insert(program_hash, ProgramInfo {bytecode: program, configuration_interface, program_deploy_key: program_deploy_key.clone(), ref_counter: 0});
  ...
  }: _(RawOrigin::Signed(sig_req_account.clone()), program_deploy_key, KeyVisibility::Public, programs_info)
  verify {
  ...
  prune_registration {
    ...
    Programs::<T>::insert(program_hash, ProgramInfo {bytecode: program, configuration_interface, program_deploy_key: program_deploy_key.clone(), ref_counter: 1});
    let sig_req_account: T::AccountId = whitelisted_caller();
    let balance = <T as pallet_staking_extension::Config>::Currency::minimum_balance() * 100u32.into();
    let _ = <T as pallet_staking_extension::Config>::Currency::make_free_balance_be(&sig_req_account, balance);
      <Registering<T>>::insert(&sig_req_account, RegisteringDetails::<T> {
        program_deploy_key: sig_req_account.clone(),
        confirmations: vec![],
  ...
  .unwrap();
  let sig_req_account: T::AccountId = whitelisted_caller();
    Programs::<T>::insert(program_hash, ProgramInfo {bytecode: program, configuration_interface: configuration_interface.clone(), program_deploy_key: program_deploy_key.clone(), ref_counter: 0});
    Programs::<T>::insert(new_program_hash, ProgramInfo {bytecode: new_program, configuration_interface, program_deploy_key: program_deploy_key.clone(), ref_counter: o as u128});
    let balance = <T as pallet_staking_extension::Config>::Currency::minimum_balance() * 100u32.into();
    let _ = <T as pallet_staking_extension::Config>::Currency::make_free_balance_be(&sig_req_account, balance);
    <Registered<T>>::insert(
        &sig_req_account,
