use super::*;
use mock::{
	new_test_ext, Call, Example, ExampleCall, Event as TestEvent, FreeTx, Balances, Origin, System
};
...
fn no_error_on_child_call_error() {
	new_test_ext().execute_with(|| {
		...
		let expected_result = Err(DispatchError::Module(ModuleError { index: 2, error: [0, 0, 0, 0], message: None }));
